...
## 8. Tournaments (Phase 7)

### tournaments
**Purpose**: Store tournament configurations and state for Sit-n-Go and scheduled tournaments.
...
- `tournament_type`: Type (sit_and_go or scheduled)
- `config`: JSONB containing full TournamentConfig (blind levels, prize structure, etc.)
- `state`: Current state (registering → running → finished/cancelled)
- `buy_in`: Buy-in amount in chips
- `min_players`: Minimum players required to start
- `max_players`: Maximum players allowed
- `starting_stack`: Initial chip stack for each player
- `registered_count`: Cached count of registered players
- `current_level`: Current blind level (1-indexed)
- `level_started_at`: When the current blind level started
- `created_at`: Tournament creation timestamp
- `started_at`: Tournament start timestamp
- `finished_at`: Tournament completion timestamp
...
**Purpose**: Track player registrations and results for tournaments.
...
- `chip_count`: Current chip count (updated during play)
- `finish_position`: Final position (1 = winner, NULL if still playing)
- `prize_amount`: Prize won (NULL if out of the money)
- `registered_at`: Registration timestamp
- `finished_at`: Elimination/finish timestamp
...
SELECT username, chip_count, finish_position, prize_amount
FROM tournament_registrations
WHERE tournament_id = $1
ORDER BY finish_position NULLS LAST, chip_count DESC;

-- Get active tournaments
SELECT id, name, state, registered_count, max_players
FROM tournaments
WHERE state = 'registering'
ORDER BY created_at DESC;

-- Get tournament standings (final results)
SELECT username, finish_position, prize_amount
FROM tournament_registrations
